...
import SimplexNoise from '../../services/perlin-noise'
...
class Background extends React.Component {
  ...
  componentWillUnmount() {
    const { intervalVar } = this.state
    window.clearInterval(intervalVar)
  }

  render() {
    const rand = (add = 0) => this.simplex.noise(add * 10, Date.now() / 200000)
    const viewWidth = window.innerWidth
    const viewHeight = window.innerHeight
    const maxDimension = Math.max(viewHeight, viewWidth)

    let cx; let cy; let rx; let ry; let px; let py
    // circle
    const cr = maxDimension * 3

    // rect
    const rSize = maxDimension * 6
    const rRot = rand(1) * 12.5

    // polygon
    const pWidth = maxDimension * 6 * 1.1547
    const pHeight = maxDimension * 6
    const pRot = rand(2) * 5
    ...
    return (
      ...
      >
        ...
        <g id="Shapes" data-name="Shapes">
          ...
          <rect
            className="cls-3"
            x={-(rSize / 2) + (viewWidth / 2)}
            y={-(rSize / 2) + (viewHeight / 2)}
            width={rSize}
            height={rSize}
            style={
              {
                transform: `translate(${rx}px, ${ry}px) rotate(${rRot}deg)`,
              }
            }
          />
          <polygon
            className="cls-4"
            points={`${viewWidth / 2} ${-pHeight / 2 + viewHeight / 2} ${-pWidth / 2 + viewWidth / 2} ${pHeight / 2 + viewHeight / 2} ${pWidth / 2 + viewWidth / 2} ${pHeight / 2 + viewHeight / 2} ${viewWidth / 2} ${-pHeight / 2 + viewHeight / 2}`}
            style={
              {
